...
from keyboards import courses_keyboard, confirm_keyboard, phone_request_keyboard
...
async def course_chosen(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(
        ...
        f"Ro'yxatdan o'tish uchun to'liq ismingizni yuboring 👇"
    )
...
async def name_entered(message: Message, state: FSMContext):
    if not message.text or len(message.text.strip()) < 3:
        await message.answer("Iltimos, to'liq ismingizni to'g'ri kiriting.")
        return
...
async def confirmed(callback: CallbackQuery, state: FSMContext):
    ...
    checkout_link = generate_checkout_link(student_id)

    await callback.message.edit_text(
        f"To'lovni amalga oshirish uchun havolani bosing 👇\n\n{checkout_link}"
    )
    await callback.answer()
    await state.clear()
...
async def rejected(callback: CallbackQuery, state: FSMContext):
    await state.set_state(Registration.entering_name)
    await callback.message.edit_text("Iltimos, to'liq ismingizni qaytadan yuboring 👇")
    await callback.answer()
